...
float emisyjnosc = 1.0; // Domyslna emisyjnosc
...
void setup() {
  ...
  lcd.print("Emisyjnosc: ");
  lcd.print(emisyjnosc);  // Wyświetl początkową wartość emisyjności
}


void loop() {
  // Odczyt temperatury z czujnika
  float temperaturaObiektu = mlx.readObjectTempC();  // Odczyt temperatury obiektu

  // Wyświetl temperaturę na wyświetlaczu
  lcd.setCursor(6, 0);  // Ustaw kursor po "Temp: " (6 kolumna w pierwszej linii)
  lcd.print(temperaturaObiektu);  // Wyświetl temperaturę
  ...
  lcd.print(emisyjnosc, 2);  // Wyświetl wartość emisyjności z dwoma miejscami po przecinku
