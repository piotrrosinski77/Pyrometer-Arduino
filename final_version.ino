...
void loop() {
  ...
  if (!BTN4V) {
    tempScale = (tempScale + 1) % 3; // Cycle through 0, 1, 2 (Celsius, Fahrenheit, Kelvin)
  ...
  float displayTemp = ObjTemp;
  char scaleLabel = 'C';
  bool showDegreeSymbol = true; // Flag to show/hide degree symbol

  if (tempScale == 1) {
    displayTemp = ObjTemp * 9.0 / 5.0 + 32.0; // Celsius to Fahrenheit
    scaleLabel = 'F';
  } else if (tempScale == 2) {
    displayTemp = ObjTemp + 273.15; // Celsius to Kelvin
    scaleLabel = 'K';
    showDegreeSymbol = false; // No degree symbol for Kelvin
  }
  ...
  lcd.setCursor(0, 0);
  lcd.print("Temp:");
  lcd.print(displayTemp);       // Show temperature
  if (showDegreeSymbol) {
    lcd.print((char)223);       // Degree symbol only for Celsius and Fahrenheit
  }
